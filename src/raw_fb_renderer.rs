...
impl RawFBRenderer {
	pub fn new(device: &wgpu::Device, swap_desc: &wgpu::SwapChainDescriptor) -> Result<Self, String> {
		let vs_src = include_str!("shaders/mmfb_copy.vert");
		let fs_src = include_str!("shaders/mmfb_copy.frag");
	...
	pub fn render(&mut self, mio: &mut FmMemoryIO, queue: &wgpu::Queue, command_encoder: &mut wgpu::CommandEncoder, framebuffer: &wgpu::SwapChainTexture) {
		let mut offset = 0;
